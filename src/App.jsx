...
export default function App() {
  ...
  // Supabase is the single source of truth for portal data.
  // The old /api/db JSON backend is no longer used for persistence.
  useEffect(() => {
    let cancelled = false;
  ...
  function uuidOrNew(value) {
    return isUuid(value) ? value : newUuid();
  }
  ...
  function toDbDate(value, fallbackOffsetDays = 0) {
    ...
    const match = text.match(/^(\d{1,2})\s+([A-Za-z]{3,9})$/);
    if (match) {
      const months = [
      ...
      const month = months.findIndex((m) =>
        m === match[2].toLowerCase().slice(0, 3)
      );
      if (month >= 0) {
        const candidate = new Date(base.getFullYear(), month, Number(match[1]));
  ...
  function timetableRowsFromState(timetables) {
    const rows = [];

    Object.entries(timetables || {}).forEach(([teacherId, week]) => {
      DAYS.forEach((day) => {
        const slots = Array.isArray(week?.[day]) ? week[day] : [];

        slots.forEach((slot, index) => {
          if (!slot) return;

          const timeText = slot.time || PERIOD_TIMES[index] || "";
  ...
  async function replaceTimetableRows(rows) {
    ...
    const wanted = new Set(
      rows.map((row) => `${row.teacher_id}|${row.day}|${row.period}`)
    );

    const idsToDelete = (existing || [])
      .filter((row) => !wanted.has(`${row.teacher_id}|${row.day}|${row.period}`))
      .map((row) => row.id)
      .filter(Boolean);
  ...
  async function saveAdminData(currentDb, adminId) {
    const teachers = currentDb.teachers.map(({ password, ...teacher }) => ({
      id: teacher.id,
      name: teacher.name,
    ...
    const admins = currentDb.admins.map(({ password, ...admin }) => ({
      id: admin.id,
      name: admin.name,
      role: admin.role || "Coordinator",
      teacher_id: admin.teacherId ?? admin.teacher_id ?? null,
      auth_user_id: admin.auth_user_id || null,
      created_at: admin.created_at || undefined,
    }));

    // UI-only isNew is intentionally not written; it is not a column in Supabase.
    const substitutions = currentDb.substitutions.map((s) => ({
      id: uuidOrNew(s.id),
      date: toDbDate(s.date),
      day: s.day,
      period: Number(s.period),
      class: s.class || "",
      absent_teacher_id: s.absentTeacherId ?? s.absent_teacher_id,
      replacement_teacher_id:
        s.replacementTeacherId ?? s.replacement_teacher_id,
      created_by: adminId || null,
    }));

    const examDuties = currentDb.examDuties.map((e) => ({
      id: uuidOrNew(e.id),
      teacher_id: e.teacherId ?? e.teacher_id,
      exam_name: e.examName ?? e.exam_name,
      exam_date: toDbDate(e.date ?? e.exam_date, 1),
      ...splitTimeRange(e.time),
      room: e.room || null,
      class: e.class || "",
      subject: e.subject || "",
      duty_type: e.dutyType ?? e.duty_type ?? "Invigilation",
      created_by: adminId || null,
    }));

    const notices = currentDb.notices.map((n) => ({
      id: uuidOrNew(n.id),
      title: n.title,
    ...
    const queries = currentDb.queries.map((q) => {
      const dbId = uuidOrNew(q.id);
      queryIds.set(q.id, dbId);
      const teacher = currentDb.teachers.find(
        (t) => t.id === (q.teacherId ?? q.teacher_id)
      );

      return {
        id: dbId,
        teacher_id: q.teacherId ?? q.teacher_id,
        subject: q.subject || teacher?.subject || "General",
        message: q.message,
    ...
    // Upsert reference/admin rows first.
    const results = await Promise.all([
      supabase.from("teachers").upsert(teachers),
      supabase.from("admins").upsert(admins),
      supabase.from("substitutions").upsert(substitutions),
      supabase.from("exam_duties").upsert(examDuties),
      supabase.from("notices").upsert(notices),
      supabase.from("queries").upsert(queries),
    ]);

    const failed = results.find((result) => result.error);
    if (failed) throw failed.error;

    // Timetables use one row per teacher/day/period, so replace that set.
    await replaceTimetableRows(timetableRowsFromState(currentDb.timetables));

    // Rebuild replies from the nested UI representation.
    const replyRows = currentDb.queries.flatMap((q) => {
      const queryId = queryIds.get(q.id) || uuidOrNew(q.id);
      return (q.replies || []).map((r) => ({
        query_id: queryId,
        from_role: r.from || "Admin",
        text: r.text,
        created_at: r.createdAt || new Date().toISOString(),
      }));
    });

    const { error: clearRepliesError } = await supabase
      .from("query_replies")
      .delete()
      .not("query_id", "is", null);

    if (clearRepliesError) throw clearRepliesError;
  ...
  async function saveTeacherData(currentDb, teacherId) {
    const mine = currentDb.queries.filter(
      (q) => (q.teacherId ?? q.teacher_id) === teacherId
    );

    const queryRows = mine.map((q) => ({
      id: uuidOrNew(q.id),
      teacher_id: teacherId,
      subject:
        q.subject ||
        currentDb.teachers.find((t) => t.id === teacherId)?.subject ||
        "General",
      message: q.message,
      resolved: Boolean(q.resolved),
      created_at: q.createdAt ?? q.created_at ?? new Date().toISOString(),
    ...
    const idMap = new Map();
    mine.forEach((q, i) => idMap.set(q.id, queryRows[i].id));

    if (queryRows.length) {
      const { error } = await supabase.from("queries").upsert(queryRows);
      if (error) throw error;
    }
  }

  // Persist local React state back into Supabase after the initial load.
  useEffect(() => {
    if (!dbLoaded) return;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const admin = db.admins.find(
          (item) =>
            item.auth_user_id === authId || item.id === authLoginId
        );

        if (admin) {
          await saveAdminData(db, admin.id);
        } else {
          const teacherId =
            db.teachers.find((item) => item.auth_user_id === authId)?.id ||
            authLoginId;

          if (db.teachers.some((item) => item.id === teacherId)) {
            await saveTeacherData(db, teacherId);
          }
        }
      } catch (e) {
...
function TeacherQueries({ db, setDb, teacher, showToast }) {
  ...
  const submit = () => {
    ...
    const q = { id: newUuid(), teacherId: teacher.id, subject: teacher.subject, message, replies: [], resolved: false };
...
function AdminSubstitutions({ db, setDb, showToast }) {
  ...
  const submit = () => {
    ...
    const s = { id: newUuid(), date: new Date().toISOString().slice(0, 10), day: form.day, period: parseInt(form.period), class: form.class, absentTeacherId: form.absentTeacherId, replacementTeacherId: form.replacementTeacherId, isNew: true };
      ...
      <div>
        ...
        <Card className="p-4 grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#1E4E8C] bg-white" />
          </div>
...
function AdminNotices({ db, setDb, showToast }) {
  ...
  const submit = () => {
    ...
    const n = { id: newUuid(), title: form.title, date: new Date().toISOString(), priority: form.priority };
